...
export default function Receitas({route}) {
  ...
  async function handleAdd(){
    let uid = usuario.uid;
    id_mov = await firebase.database().ref('Movimentacoes').child(uid).push().key;
    ...
    await contas.once('value').then((snapshot)=>{
      ...
      let totReceitasContas= parseFloat(snapshot.val().totReceitasContas);
      saldoConta += parseFloat(valor);
      totReceitasContas += parseFloat(valor);
      contas.child('saldoConta').set(saldoConta);
      contas.child('totReceitasContas').set(totReceitasContas);
    });

    //cuidando dos valores da tabela useres
    ...
    await users.once('value').then((snapshot)=>{
      ...
      let totReceitas = parseFloat(snapshot.val().totReceitas);
      saldoUser += parseFloat(valor);
      totReceitas += parseFloat(valor);
  ...
  <TouchableWithoutFeedback onPress={()=>Keyboard.dismiss()}>
    <Background>
      <SafeAreaView style={{alignItems: 'center'}}>
          ...
          <PickerCategoria onChange={setTipoCategoria} tipoCategoria={tipoCategoria} keyC={route.params?.key} keyM={id_mov}/>
